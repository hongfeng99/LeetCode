...
class Solution {
    public boolean isNumber(String s) {
        ...
        if(ptr==n) //假如不包含e与E
        {
        	...
        	{
        		if(countofplus>0)      //如果不是以+或者-开头，但是中间部分含有+或者-
        		{
        			return false;
        		}
        		return true;
        ...
        else         //如果包含e或者E,此时ptr就指向e或者E
        {
        	...
        	if(s.charAt(0)=='+'||s.charAt(0)=='-')
        	{
        		...
        		if(p1==1&&n1==2)  //如果s1是+.或者-.
        		{
        			return false;
        		}
        		if(n1==1)
        		{
        			return false; //如果e前面只有一个.
        		}
        	}
        	else
        	{
        		if(n1==0)
        		{
        			return false;  //如果形式类似于e9
        		}
        		if(n1==1&&s.charAt(0)=='.')
        		{
        			return false;  //如果形式类似于.e1
        		}
        		if(countofplus>0)
        		{
        			return false;
